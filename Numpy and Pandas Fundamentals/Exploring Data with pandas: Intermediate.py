...
selected_rows = f500.loc[f500['country']=='Japan',:]
...
for c in countries:
    country = f500[f500['country']==c]
    country = country.sort_values('employees',ascending=False)
    country_row = (country.iloc[0])['company']
    top_employer_by_country[c]=country_row

## 12. Challenge: Calculating Return on Assets by Country ##

f500['roa'] = f500['profits']/f500['assets']
top_roa_by_sector = {}
sector_name = f500['sector'].unique()
for s in sector_name:
    rows = f500[f500['sector']==s]
    sort_row = rows.sort_values('roa',ascending=False)
    max_sector = sort_row.iloc[0]
    max_sector = max_sector['company']
    top_roa_by_sector[s] = max_sector
