...
def find_variability(array):
    array_mean = sum(array)/len(array)
    distances = []
    for i in range(len(array)):
        distances.append(array[i]-array_mean)
    return sum(distances)/len(distances)
avg_distance = find_variability(C)
print(avg_distance)

## 3. Mean Absolute Deviation ##

C = [1,1,1,1,1,1,1,1,1,21]
def find_abs_deviation(array):
    array_mean = sum(array)/len(array)
    absolute_distances = []
    for i in range(len(array)):
        absolute_distances.append(abs(array[i]-array_mean))
    return sum(absolute_distances)/len(absolute_distances)
mad = find_abs_deviation(C)
...
def variance(array):
    ...
    squared_distances = []
    for i in range(len(array)):
        squared_distances.append((array[i]-array_mean)**2)
